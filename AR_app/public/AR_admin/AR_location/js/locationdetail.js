                ...
                for (const mdlid in models) {
                    ...
                    detailsHtml += `
                        <p>
                            ...
                            <strong><a href="../../AR_admin/AR_napisy/soundlist.html?mdltext=${model.mdltext}">音声ファイル</a></strong><br>
                            ${model.soundfiles.length > 0 ? model.soundfiles.map(file => `<span>${file}</span><br>`).join('') : 'なし'}<br>
                            <strong><a href="../../AR_admin/AR_sound/napisylist.html?mdlsound=${model.mdlsound}">テキストファイル</a></strong><br>
